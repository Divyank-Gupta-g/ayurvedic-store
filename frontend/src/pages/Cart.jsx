...
const Cart = () => {
    const {products, currency, cartItems, removeFromCart, updateCartItem, navigate, 
      cartItemCount, totalCartValue, axios, user, setCartItems} = useAppContext();
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">
                    Shopping Cart <span className="text-sm text-primary">{cartItemCount()} items</span>
                </h1>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <select
                      onChange={e => setPaymentMethod(e.target.value)} className="w-full border border-gray-300 bg-white px-3 py-2 mt-2 outline-none">
                ...
                <div className="text-gray-500 mt-4 space-y-2">
                    <p className="flex justify-between">
                        <span>Price</span><span>{currency}{totalCartValue()}</span>
                    </p>
                    <p className="flex justify-between">
                        <span>Shipping Fee</span><span className="text-green-600">Free</span>
                    </p>
                    <p className="flex justify-between">
                        <span>Tax (2%)</span><span>{currency}{totalCartValue() * 2 / 100}</span>
                    </p>
                    <p className="flex justify-between text-lg font-medium mt-3">
                        <span>Total Amount:</span>
                        <span>{currency}{totalCartValue() + (totalCartValue() * 2 / 100)}</span>
                    </p>
                </div>

                <button 
                  // onClick={() => {paymentMethod === 'COD' ? navigate('/orderPlaced') : navigate('/checkout'); scrollTo(0,0);}}
                  onClick={placeOrder}
