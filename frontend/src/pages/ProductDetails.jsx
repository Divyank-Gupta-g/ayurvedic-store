    ...
    return product && (
        <div className="mt-12">
            <p>
                ...
                <Link to={`/products/${product.category}`} className="hover:underline">{product.category}</Link> /
            ...
            <div className="mt-20">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {relatedProducts.filter((product) => product.inStock).map((item) => (
                        <div key={item._id} onClick={() => navigate(`/products/${item.category}/${item._id}`)} className="border border-gray-300 rounded-lg p-4 cursor-pointer hover:shadow-lg transition">
                            <img src={item.image[0]} alt={item.name} className="w-full h-48 object-cover mb-4" />
                            <h3 className="text-lg font-medium">{item.name}</h3>
                            <p className="text-gray-500/70 line-through">MRP: {currency}{item.price}</p>
                            <p className="text-xl font-medium">MRP: {currency}{item.offerPrice}</p>
                        </div>
                    ))}
                </div>
                
                <button onClick={() => {navigate("/products"); scrollTo(0, 0)}} className="mt-8 px-6 py-3 bg-primary text-white rounded hover:bg-primary-dull transition cursor-pointer">See All Products</button>
            </div>
        </div>
    );
};
